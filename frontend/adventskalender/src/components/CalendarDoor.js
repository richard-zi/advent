...
const CalendarDoor = ({ day, isOpen, onOpen, contentPreview }) => {
  ...
  return (
    <div 
      className={`aspect-square rounded-lg flex flex-col items-center justify-center cursor-pointer transition-all duration-300 transform hover:scale-102 border text-sm ${
        isOpen 
      ...
      {isOpen ? (
        <div className="flex flex-col items-center justify-center p-1 w-full h-full">
          {contentPreview && contentPreview.type === 'text' && (
            <p className="text-xs sm:text-sm text-gray-600 text-center overflow-hidden">{getPreviewText(contentPreview.data)}</p>
          )}
        ...
        <div className="w-full h-full flex flex-col items-center justify-center">
          <span className="font-bold text-xl sm:text-2xl text-gray-700">{day}</span>
          <div className="mt-1 w-4 h-0.5 bg-red-600 rounded-full"></div>
        </div>
      )}
    </div>
  );
