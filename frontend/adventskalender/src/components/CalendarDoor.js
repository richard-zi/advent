      ...
      {isOpen ? (
        <div className="flex flex-col items-center justify-center p-1 w-full h-full">
          {contentPreview && contentPreview.type === 'text' && (
            <p className="text-[0.6rem] sm:text-xs md:text-sm text-gray-600 text-center overflow-hidden">{getPreviewText(contentPreview.data)}</p>
          )}
          {contentPreview && contentPreview.type === 'video' && (
            <div className="w-full h-full flex items-center justify-center bg-gray-100 rounded-md">
              <svg className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="w-full h-full flex flex-col items-center justify-center">
          <span className="font-bold text-xl sm:text-2xl md:text-3xl text-gray-700">{day}</span>
